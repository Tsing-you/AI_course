...
def preprocess_data(df):
    ...
    # 新增血氧变化率
    df_ts["blood_oxygen_diff"] = df_ts["blood_oxygen"].diff().abs().fillna(0).round(1)

    # 新增血氧移动平均
    df_ts["ma_bo_3"] = df_ts["blood_oxygen"].rolling(3, min_periods=1).mean().round(1)
...
class HeartAnalysisApp:
    ...
    def update_display(self):
        ...
        # 构建评估指标字典
        metrics = {
            'health_score': analysis.get('健康评分', 0),
            'risk_prob': analysis.get('心血管风险概率', 0),
            'oxygen_abnormal': analysis.get('血氧异常次数', 0),
            'hr_anomalies': len([a for a in self.anomalies if '心率' in a]),
            'sdnn': analysis.get('SDNN', 0),
            'lf_hf': analysis.get('LF/HF', 0),
            'trend': analysis.get('趋势斜率', 0)
        }
        ...
        if metrics['health_score'] >= 85:
            recommendations.append("整体健康状况优秀，各项指标维持在理想范围")
        elif metrics['health_score'] >= 70:
            recommendations.append("整体健康状况良好，但存在少量需关注指标")
        else:
            recommendations.append("健康状况需重点关注，建议全面复查")

        # 心血管风险评估
        if metrics['risk_prob'] < 0.2:
            recommendations.append("心血管风险处于低水平")
        elif metrics['risk_prob'] < 0.5:
            recommendations.append("存在中度心血管风险，需注意生活方式调整")
        else:
            recommendations.append("心血管风险较高，建议咨询专业医生")

        # 血氧专项评估
        if metrics['oxygen_abnormal'] == 0:
            recommendations.append("血氧饱和度维持稳定，呼吸功能良好")
        elif metrics['oxygen_abnormal'] < 5:
            recommendations.append("偶发血氧异常，建议关注环境因素影响")
        else:
            recommendations.append("频繁血氧异常，需排查呼吸系统问题")

        # HRV自主神经评估
        if metrics['sdnn'] > 50 and metrics['lf_hf'] < 3:
            recommendations.append("自主神经调节功能良好，压力水平适中")
        elif metrics['lf_hf'] >= 3:
            recommendations.append("交感神经兴奋性偏高，建议加强压力管理")

        # 趋势分析
        if metrics['trend'] > 1:
            recommendations.append("心率呈持续上升趋势，需注意身体负荷")
        elif metrics['trend'] < -1:
            recommendations.append("心率呈下降趋势，需关注心血管功能")

        # 生成带智能推荐的评估报告
        for i, rec in enumerate(recommendations, 1):
            icon = "✅" if i <= 2 else "⚠️" if i <=4 else "❗"
            analysis_str += f"{icon} {rec}\n"

        # 动态生成健康建议标签
        health_tags = []
        if metrics['health_score'] > 85:
            health_tags.append("🏅 健康模范")
        if metrics['oxygen_abnormal'] == 0:
            health_tags.append("🫁 呼吸健康")
        if metrics['lf_hf'] < 2.5:
            health_tags.append("🧘‍♂️ 压力平衡")
            
        if health_tags:
            self.status_label.config(
                text=" | ".join(health_tags), 
                foreground="green" if len(health_tags)>=3 else "orange"
            )
    ...
    def send_message(self):
        ...
        try:
            ...
            ai_reply = self._call_ai_api(messages, model="glm-4-flash")#glm-4-plus
    ...
    # 统一的AI接口调用方法
    def _call_ai_api(self, messages, model="glm-4-flash"):#glm-4-plus
        """统一的AI接口调用方法"""
